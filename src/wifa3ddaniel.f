...
C--------------------------------------------------------------
C
      WRITE(PDATE,1) '2022/01/01'
      WRITE(PVER ,1) 'Ver:2022R1'
 1    FORMAT(10A)
C
      NYM=NY-1
      NZM=NZ-1
...
C
      CALL FINDFILENAME
      IF(MYID.EQ.IPRN) PRINT *, 'HELLO1'
C
      CALL MODINP
      IF(MYID.EQ.IPRN) PRINT *,	'HELLO2'
C
      CALL INITMPI(FILRST,FILRES,NPRO)
      IF(MYID.EQ.IPRN) PRINT *,	'HELLO3'
C
      CALL INITFIELD
      IF(MYID.EQ.IPRN) PRINT *,	'HELLO4'
C
...
C#############################################################
      ...
      DOUBLE PRECISION P1,P2,P3,RESL,TMP,RSM
C     
...
C
        IF(NPRO.GT.1)THEN
          CALL MPI_ALLREDUCE(RESL,TMP,1,MPI_DOUBLE_PRECISION,MPI_SUM,
     *                       MPI_COMM_WORLD,IERR)
          RESL=TMP
        ENDIF
C
...
C
      ...
      READ(5,*)  NPC(J)
      READ(5,11) CTMP 
      READ(5,*) (WDS(J,K),PCS(J,K),PCP(J,K),PCR(J,K),K=1,NPC(J))
      END DO
C
      CLOSE(5)
C
...
C
      ...
      END DO
      IF(NPRO.GT.1)THEN
        CALL MPI_ALLREDUCE(TMPA,TMP,1,MPI_DOUBLE_PRECISION,MPI_MAX,
     *                     MPI_COMM_WORLD,IERR)
        TMPA=TMP 
        CALL MPI_ALLREDUCE(TMPB,TMP,1,MPI_DOUBLE_PRECISION,MPI_MIN,
     *                     MPI_COMM_WORLD,IERR)
        TMPB=TMP 
      ENDIF
      VALMAX=TMPA
...
C##########################################################
      ...
      INCLUDE 'index.inc'
      INCLUDE 'mpif.h'
      DOUBLE PRECISION RT(NPRO),TMP,ALL
C
      TMP=NXMM*NYMM*NZMM
...
C############################################################
      ...
      INCLUDE 'rcont.inc'
      INCLUDE 'mpif.h'
      DOUBLE PRECISION TMP,TVISMAX,DN,TI,TA,TII,TAA,YMIN(6),YMAX(6)
C
      DO I=IXBN,IXED
        II=I-IREF
        DO J=2,NYM 
...
C#######################################################################
      ...
      INCLUDE 'wind.inc'
C
      DOUBLE PRECISION TMPAR,ROTA,U2,R3,THRUST,ROTR,PI2,START,FINISH
      DOUBLE PRECISION VP(4),TMPAF,TMPTF,TMPRR,DR
      DOUBLE PRECISION THETA,COSTH,SINTH,DTH,TMPWTE,TMPTE
      DOUBLE PRECISION WTCP,WTCT,WTLA,WTCG,U3
      DIMENSION IJK(4)
C
      PI2=PI*PI
      DO I=1,NXYZ
...
C
       ...
       U2=VSCA(IWT)*VSCA(IWT)
       U3=U2*VSCA(IWT)
C
       TMP=HALF*DENSIT*ROTA
       WTCP=POWER(IWT)/(TMP*U3)
       WTCT=THRUST/(TMP*U2)
       WTLA=RPM(IWT)*ROTR/VSCA(IWT)*PI*THIRD*TENTH
       WTCG=(WTCP**EXPCP(ITY))*(WTCT**EXPCT(ITY))*(WTLA**EXPLA(ITY))
       TMPWTE=WTCG*CGSCA(ITY)*TMP*U3
C
C      RADIAL DISTRIBUTION OF BLADE LOADING
C
       IF(LBLADE)THEN
C
         TMPAF=THRUST/ROTA
...
C
      DO IWT=1,NWT
        ...
        IF(IWTY(IWT).LT.1.OR.IWTY(IWT).GT.NWTD)THEN
          IF(MYID.EQ.IPRN) WRITE(*,*) "IWTY:", IWTY(IWT)
...
C
      IF(NPRO.GT.1)THEN
        CALL MPI_ALLREDUCE(IWS,ISUM,NWT,MPI_INTEGER,MPI_SUM,
     *                     MPI_COMM_WORLD,IERR)
        DO IWT=1,NWT
          IF(ISUM(IWT).EQ.0)THEN
           IF(MYID.EQ.IPRN) THEN
